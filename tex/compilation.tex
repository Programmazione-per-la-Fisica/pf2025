...
\begin{frame}{Header and source files}
  ...
  \item For a given software component, a \textul{typical} situation foresees:
    ...
    \item One \textbf{header} file containing free function declarations, class
      definitions with method declarations, template definitions
      \begin{itemize}[<.->]
      \item The contents of the header file represent the \textbf{interface} of
        the component
      \end{itemize}
    \item One \textbf{source} file containing definitions of free functions, methods
...
\begin{frame}[fragile]{Header and source files \insertcontinuationtext}
  ...
  \begin{itemize}[<+->]
    \item We will name header files using the \code{.hpp} extension
      \begin{itemize}[<.->]
      \item Header files are \textbf{never directly compiled}, only source files are
      \end{itemize}
    \item We will name source files using the \code{.cpp} extension
      \begin{itemize}[<.->]
      \item \textbf{A source file corresponds to a translation unit} and vice versa
      \end{itemize}
...
\begin{frame}[fragile]{Include guards}
  ...
  \item A header (file) may be included, directly or indirectly, multiple times
    ...
    \begin{codeblock}
#include "result.hpp"
#include "regression.hpp" // regression.hpp already includes result.hpp\end{codeblock}
...
\begin{frame}{Build systems}
  ...
  \item When a project includes more than a few files, keeping track of
    ...
    \item Usually multiple variants of a project are needed: one for debug,
    one for testing with code coverage, one for release, one for measuring
...
target_link_libraries(mandelbrot_sfml PRIVATE sfml-graphics)
  ...
  \item You won't need to become a CMake expert: we will provide
  \code{CMakeLists.txt} drafts for a following lab and for the exam project
\end{itemize}
\end{frame}

\begin{frame}{Compilation and testing with CMake}
  \begin{itemize}[<+->]
  \item Before compiling a program one must generate a build area
    \begin{itemize}[<.->]
    \item Here, the build area is generated under the \code{build/} folder
    \end{itemize}
    \begin{shellblock}{
...
\ldots    
  ...
  \item Then the actual compilation command can be issued
    \begin{shellblock}{
\$ cmake --build build --config Debug

[6/6] Linking CXX executable Debug/regression.t
}\end{shellblock}
  \item Finally, CMake also allows to trigger all the project tests using a
  single command
    \begin{shellblock}{
