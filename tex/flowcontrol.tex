...
std::vector<int> numbers\{1, 42, -12\}; // a vector with 3 int\textquotesingle{}s\end{codeblock}
  ...
  \item<2-> To know the number of elements in a vector, call the \code{size}
    \textit{method} (see later)
    \begin{codeblock}
std::cout << numbers.size(); // prints 3\end{codeblock}

  \item<3-> To append an element to a vector, call the \code{push_back} \textit{method}
    \begin{codeblock}
...
\ddd numbers[numbers.size()] \ddd        // undefined behaviour, beware!\end{codeblock}
  ...
  \item<2-> To iterate over all the elements of a vector, \underline{for the moment}
...
\ddd
double x\{\ddd\};
std::sqrt(x); // note: always qualify cmath functions with std::
std::pow(x, .5);
std::sin(x);
std::log(x);
