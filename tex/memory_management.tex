  ...
  \begin{columns}
    \begin{column}{.75\textwidth}
      ...
      \item Heap
        ...
        \end{itemize}
      \item Global and \textit{static} data
        \begin{itemize}
        \item literals and variables
        \item initialized and uninitialized (initially set to 0)
        \end{itemize}
...
\begin{frame}{Dynamic memory allocation}
  ...
  \item<3-> The lifetime of an object (or of an array of objects) on the heap is
    ...
    \item \code{delete} expression (for an array: \code{delete []})
      \begin{itemize}
      \item run the object(s) destructor
...
auto copy = original;\end{codeblock}
...
v2 = v1;\end{codeblock}
  ...
  \item<3-> The compiler-generated operations may not be correct,
    especially in presence of managed resources
...
\{
 ...
 public:
  DynamicArray(int n, int v = 0) : m_size\{n\}, m_data\{new int[m_size]\}
...
class SmartPointer \{
 ...
 public:
  ...
  \visible<4->{Pointee* operator\alert<4>{->}() const \{ return m_p; \}
  Pointee& operator\alert<4>{*}() const \{ return *m_p; \}}
\};
...
class UniquePtr \{
 ...
 public:
  ...
  Pointee* operator->() const \{ return m_p; \}
  Pointee& operator*() const \{ return *m_p; \}
\};\end{codeblock}
...
\begin{frame}{Move semantics}
  ...
  \begin{itemize}
  \item It allows to properly \textit{move} (i.e. transfer) the responsibility
    of a resource from one managing object to another
...
auto q = std::move(p);\end{codeblock}
...
\begin{frame}{Controlling moving \insertcontinuationtext}
  ...
  \item<3-> A move should leave the original object in a \textit{``valid but
    ...
    \end{itemize}
  \item<4-> Move operations should be declared \code{noexcept}, which
    approximately means ``this function/operation doesn't fail''
...
\begin{frame}{Associative unordered containers}

  \begin{itemize}
  \item They contain values (\code{set} and \code{multiset}) or
    key-value pairs (\code{map} and \code{multimap})
  \item Search, removal and insertion have constant complexity
  \item A \textit{hash} function, associated to the container, when
    applied to the key, returns the position inside the container
  \end{itemize}
  \includegraphics[width=\textwidth]{images/bucket-groups.png}
\end{frame}
