...
for (auto const& s : shapes) \{ s.position(); \} // wish
...
\uncover<6->{  \alert<6>{virtual} Point position() const \alert<6>{= 0};\only<6>{ // \alert{pure virtual function}}}
\uncover<2->{\};}
...
struct Circle \alt<1>{\{ // \textit{is a} }{: }Shape \uncover<2->{\{}\only<2>{ // \alert{derived class}}
  ...
  Point position() const\only<6->{ \alert<6>{override}};
\};
...
\uncover<5->{s\alt<-7>{.}{\alert<8>{->}}position();}\only<5>{ // \alert<5>{error}}
...
struct Shape \{                     // \alert{abstract} base class
  \alert{virtual} Point position() const \alert{= 0}; // \alert{pure virtual} function
...
struct Circle : Shape \{ // derived class
  Point c;
  int r;
  Point position() const \alert{override};
\};
...
\begin{frame}{Dynamic polymorphism \insertcontinuationtext}
  ...
  \item<2-> Virtual functions support \textbf{dynamic binding} and
    ...
    \item A call to a virtual function made through a pointer/reference to a
      base class (the \textit{static} type) is dispatched at \textbf{runtime} to
      the corresponding function of the actual derived class (the
      \textit{dynamic} type) of the object behind that pointer/reference
    \item The design of a software component should ideally follow the
      \textbf{Liskov substitution principle}: a program should still behave
      correctly if an object of a base class is replaced by an object of a
      derived class
    \end{itemize}
...
\begin{frame}{Abstract base class}
...
struct Shape \{
  ...
  Shape(Point p) : p\{p\} \{\}}}
  virtual \~Shape();
  virtual Point position() const \only<-2>{= 0;}\only<3->{\alert<3>{\{ return p; \}} // \alert<3>{default implementation}}
\};
...
struct Circle : Shape \{
  ...
  \~Circle();
  \alt<-2>{Point position() const override \{ return \alt<1>{c}{p}; \}}{\alert<3>{// position() is inherited from Shape}}\only<2>{ // \alert{p is inherited}}
\};
...
struct Rectangle : Shape \{
  ...
  \~Rectangle();
  Point position() const override \{ return (\temporal<2,3>{ul}{p}{\alert<4>{Shape::position()}} + lr) / 2; \}\only<2>{ // \alert{p is inherited}}
\};\end{codeblock}
...
struct Shape \{
  ...
  Shape(Point p) : p\{p\} \{\}
  virtual \~Shape();
  virtual Point position() const // \alert{not pure; default implementation}
  \{ return p; \} 
\};
...
struct Circle : Shape \{
  ...
  // position() implementation is inherited from Shape
\};

struct Rectangle : Shape \{
  Point lr;
  Rectangle(Point p1, Point p2) : \alert{Shape\{p1\}}, lr\{p2\} \{\}
  Point position() const override \{ return (\alert{p} + lr) / 2; \} // \alert{p is inherited}
\};
}\end{codeblock}
...
\{
  ...
  virtual Point position() const \{ return p; \}
\};
...
\{
  \ddd shape.position() \ddd\only<4->{ // Point\{2., 4.\}}
\}}

\uncover<5->{void process2(\alert<6>{Shape} shape)\only<6>{ // by value}
\{
  \ddd shape.position() \ddd\only<7->{ // Point\{1., 7.\}}
\}}
...
\begin{frame}[fragile]{Keeping the base class abstract}
  ...
  \item<3-> Yet the destructor needs to be defined
    \begin{itemize}
    \item So that the base class sub-object is properly destroyed during the
      destruction of an object of a derived class
    \end{itemize}
  \end{itemize}
...
struct Shape \{
  ...
  virtual Point position() const; // non-pure virtual function
\};
...
\uncover<2->{ \alert<2>{public}:}
  Shape(Point p) : p\{p\} \{\}
  virtual ~Shape() = default;
  virtual Point \alert<6>{position}() const \{ return p; \}
...
\uncover<2->{ \alert<2>{public}:}
  ...
  ~Rectangle();
  double area() const override \{ \ddd \uncover<3->{abs(\only<3-5>{\alert{p}}\only<6->{\alert<6>{position()}}.x - \alert<3-5>{lr}.x)} \ddd \}
\};\end{codeblock}
