...
\begin{frame}[fragile]{Global objects}
...
auto print_rand()     \{ std::cout << get_rand(); \}}\end{codeblock}
      ...
      \item<3-> They have \textit{static} storage duration, i.e. they live for
...
\begin{frame}[fragile]{Global objects \insertcontinuationtext}
  \begin{itemize}
  \item<1-> Initialization
    \begin{itemize}
    \item Memory for global variables is guaranteed to be at least
  ...
  \item<2-> Warning
    \begin{itemize}
    \item Global variables make reasoning about the program more difficult
...
\begin{frame}[fragile]{Constants}
  ...
  \begin{itemize}
  \item One useful application of global variables is to define constants,
...
\begin{frame}[fragile]{Class \code{static} data members}
  ...
  \item<2-> A static data member
    ...
    \item<5-> \ldots unless it is declared \code{inline} or \code{constexpr}
...
std::cout << std::chrono::system_clock\alert{::}is_steady;\end{codeblock}
  ...
  \item But if there is an object of that type, one can use the \code{.}
...
\begin{frame}[fragile]{Class \code{static} member functions}
  ...
  \begin{itemize}
  \item A class member function (method) declared \alert{\code{static}} is not
    associated with any object of that class
